...
import com.glambiase.run.domain.LocationDataCalculator
...
class ActiveRunViewModel(
    ...
    private val shouldTrack = snapshotFlow { state.shouldTrack }
        .stateIn(scope = viewModelScope, started = SharingStarted.Lazily, initialValue = state.shouldTrack)
    ...
    fun onAction(action: ActiveRunAction) {
        when (action) {
            ...
            ActiveRunAction.OnStartAndStopRunClick -> {
                state = state.copy(
    ...
    override fun onCleared() {
        super.onCleared()
        if (!ActiveRunService.isServiceActive) runningTracker.stopObservingLocation()
    }
}